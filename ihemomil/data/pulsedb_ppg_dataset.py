...
PulseDBBPF_CLZ_NAMES = [
    "normal",
    "hypertension",
    "hypotension",
]

class PulseDBPPGDataset(MILTSCDataset):
    ...
    def __init__(self, split: str, name: str = "VitalDB", apply_transform: bool = True):
        ...
        metadata_path = "data/PulseDBBPF/{:s}_{:s}_metadata.json".format(self.dataset_name, split.upper())
    ...
    def get_time_series_collection_and_targets(self, split: str) -> Tuple[List[torch.Tensor], torch.Tensor]:
        df = pd.read_csv("data/PulseDBBPF/{:s}_{:s}.csv".format(self.dataset_name, split.upper()))
        ts_collection = []
        targets = []

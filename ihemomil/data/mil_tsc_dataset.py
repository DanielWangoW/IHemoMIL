"""
The MILTSCDataset class has the following https://github.com/JAEarly/MILTimeSeriesClassification (under Apache 2.0 License)
This file contains the implementation of an abstract class for MIL (Multiple Instance Learning) Time Series Classification (TSC) dataset.
The MILTSCDataset class is a subclass of the torch.utils.data.Dataset and abc.ABC classes. 
It provides a common interface for loading MIL TSC datasets and accessing the time series bags and their targets.

Attributes:
    - dataset_name (str): The name of the dataset.
    - split (str): The split of the dataset (e.g., train or test).
    - apply_transform (bool): Flag indicating whether to apply z-normalization to the time series bags.
    - ts_collection (List[torch.Tensor]): List of time series tensors.
    - targets (torch.Tensor): Tensor of classes (targets).
    - n_clz (int): Number of classes in the dataset.
"""

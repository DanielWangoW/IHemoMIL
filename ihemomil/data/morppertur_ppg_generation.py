"""
Generation functions for the morphological perturbation PPG dataset
...
def inject_signature(ts: np.ndarray, signature_name: str) -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    if signature_name not in MORPPERTURPPG_CLZ_NAMES:
...
def perturbation_and_save_dataset(
    dataset_name: str, split: str, signature_clzs: List[str],
) -> None:
    """
    ...
    df_ppg = pd.read_csv("../../data/MorpPerturPPG/Raw_data/{:s}_{:s}_Raw.csv".format(dataset_name, split.upper()), header=None, sep=',')
    ts_len = len(df_ppg.columns)
    n_ts_per_clz = len(df_ppg)
    ...
    for clz, signature_type in enumerate(signature_clzs):
        for item in range(n_ts_per_clz):
            # Load PPG waveform from file
            ts = df_ppg.iloc[item, :].to_numpy()
            # Inject signature
    ...
    save_dir = "../../data/MorpPerturPPG"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    data_path = "{:s}/{:s}_{:s}.csv".format(save_dir, dataset_name, split.upper())
    metadata_path = "{:s}/{:s}_{:s}_metadata.json".format(save_dir, dataset_name, split.upper())
...
def _inject_signatures_switcher(ts: np.ndarray, signature_name: str) -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    if signature_name == "scale":
        return _inject_scale(ts)
...
def _inject_flip(
    ts: np.ndarray,
    len_min: int = 32,  # 1/4 second
    len_max: int = 250,  # 2 seconds
) -> Tuple[np.ndarray, np.ndarray, Dict]:
...
def _inject_scale(
    ...
    # Inject the wandering
    ts_w_signature = ts.copy()
    ts_w_signature[signature_start:signature_end] *= factor 
    signature_details = {
        "len": int(signature_end - signature_start),
        "w:": float(factor),
    }
